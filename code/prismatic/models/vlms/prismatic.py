...
class PrismaticVLM(VLM):
    ...
    def forward(
        ...
        if input_ids.shape[1] == 1 and past_key_values is not None:
            ...
            # Transparently pass through semantics/actions if present
            return llm_output
        ...
        # Run LLM Forward --> returns CausalLMOutputWithPast (and optionally semantics/actions)!
        llm_output = self.llm_backbone(
            input_ids=None,
        ...
        # Handle ActDistill: transparently pass through semantics and actions if present
        if isinstance(llm_output, tuple) and len(llm_output) == 3:
            return llm_output  # (causal_output, semantics, actions)
        else:
            return llm_output  # Just causal_output
